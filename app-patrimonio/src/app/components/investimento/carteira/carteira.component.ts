...
export class CarteiraComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!!carteiraId) {
      this.investimentoStateService.carteira.pipe(
        mergeMap(carteiras=>{
          return this.investimentoStateService.cotacao.pipe(
            map(cotacoes=> {
              ...
              return {carteira, cotacoes, carteiras};
            })
          )
        })
      ).subscribe(result=> {
        this.carteira = result.carteira;
        this.carteiras = result.carteiras;
        this.cotacoes = result.cotacoes;
      });
    }
  }
